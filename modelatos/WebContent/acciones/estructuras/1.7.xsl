...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="OSM-ACCION">
		...
		<pagina titulo="Confirmación Eliminado">
			...
			<principal>
				<titulo icono="estructura">Confirmación Eliminado</titulo>
				<contenido>
		
					<xsl:choose>

						<xsl:when test="eliminarEstructura/exitoso='true'">
							<bloque-contenido>
								<titulo icono="confirmacion">Confirmación Eliminado</titulo>
								<contenido>
									<parrafo>La Estructura ha sido eliminada Exitosamente.</parrafo>
								</contenido>
							</bloque-contenido>
						</xsl:when>
