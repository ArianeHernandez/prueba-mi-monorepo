...
implements SourceFactory, 
    ...
    throws SourceException, MalformedURLException, IOException {
        ...
        try {
            ...
            // INICIO MODIFICACION-ITO
            if(u==null && path.indexOf("//")==0){
    			
            	String baseString;
            	// baseString = new File(ContextSourceFactory.class.getResource("/").getPath()).getParentFile().getParentFile().getAbsolutePath();
            	baseString = ContextInfo.getInstance().getRealPath(null);
            	getLogger().info("cocoon, modificacion ito. BaseString to load xsp resources set to:" + baseString );
    			File file = new File(baseString + path);
    			if(file.exists()){
    				u = file.toURL();
    			}
    		}
            // FIN MODIFICACION-ITO

            if (u != null) {
            	
            	String external = u.toExternalForm();
            	
                return resolver.resolveURI(external);
