...
public class GeneradorSIFAdminPortafolioTerceroACH implements IGeneradorSalidaSistemaCorreval {

	private String tipo_archivo = "APT-ACH-SIF";
	...
	public File generar(List<SalidaCoreCorreval> listaSalidaSistemaCorreval) {
		...
		try {
			...
			ccel2.setCellValue(new HSSFRichTextString("Cuenta"));
			ccel2.setCellStyle(styleCelda);

			HSSFCell ccel3 = rowc0.createCell((short) 3);
			ccel3.setCellValue(new HSSFRichTextString("Identificación del Cliente"));
			...
			for (SalidaCoreCorreval ssc : listaSalidaSistemaCorreval) {
				...
				cel2.setCellValue(new HSSFRichTextString(ssc.getNum_cuenta_cliente_correval()));
				cel2.setCellStyle(styleCelda);

				HSSFCell cel3 = row.createCell((short) 3);
				cel3.setCellValue(new HSSFRichTextString(ssc.getIdentificacion_cliente()));
				...
				cel6.setCellValue(ssc.getNum_identificacion_beneficiario());
				...
				cel8.setCellValue(new HSSFRichTextString(ssc.getNombre_banco_destino_beneficiario()));
				cel8.setCellStyle(styleCelda);

				HSSFCell cel9 = row.createCell((short) 9);
				cel9.setCellValue(new HSSFRichTextString(ssc.getNum_cuenta_destino_beneficiario()));
				cel9.setCellStyle(styleCelda);

				HSSFCell cel10 = row.createCell((short) 10);
				cel10.setCellValue(new HSSFRichTextString(ssc.getTipo_cuenta_destino_beneficiario()));
				cel10.setCellStyle(styleCelda);

				HSSFCell cel11 = row.createCell((short) 11);
				cel11.setCellValue(new HSSFRichTextString(ssc.getNombre_banco_origen_correval()));
				cel11.setCellStyle(styleCelda);

				HSSFCell cel12 = row.createCell((short) 12);
				cel12.setCellValue(new HSSFRichTextString(ssc.getNum_cuenta_origen_correval()));
				cel12.setCellStyle(styleCelda);

				HSSFCell cel13 = row.createCell((short) 13);
				cel13.setCellValue(new HSSFRichTextString(ssc.getTipo_cuenta_origen_correval()));
				cel13.setCellStyle(styleCelda);

				HSSFCell cel14 = row.createCell((short) 14);
				cel14.setCellValue(new HSSFRichTextString("Lote # " + ssc.getLote_origen()));
				cel14.setCellStyle(styleCelda);

				HSSFCell cel15 = row.createCell((short) 15);
				cel15.setCellValue(new HSSFRichTextString(dateformat.format(ssc.getFecha_liberacion_lote_origen())));
				cel15.setCellStyle(styleCelda);

				HSSFCell cel16 = row.createCell((short) 16);
				cel16.setCellValue(new HSSFRichTextString(ssc.getNombre_agente_comercial()));
				cel16.setCellStyle(styleCelda);

				HSSFCell cel17 = row.createCell((short) 17);
				cel17.setCellValue(new HSSFRichTextString(ssc.getEstado_retiro()));
