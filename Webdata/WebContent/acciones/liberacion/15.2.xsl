...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="OSM-ACCION">
		<javascript>liberacion/15.2.js</javascript>

		<SELECCIONAR-NEGOCIO />

		<pagina titulo="Solicitudes">
			...
			<principal>
				...
				<contenido>
					...
					<xsl:variable name="total" select="//contarCargasLiberacion/numCargas" />
					<xsl:choose>
...
<!-- 									<titulo>Tipo de carga</titulo> -->
									<titulo>Usuario Solicitante</titulo>
<!-- 									<titulo>Valor</titulo> -->
<!-- 									<titulo>N°registros</titulo> -->
								</encabezado>
								<xsl:for-each select="//obtenerCargasLiberacion/Listado/Carga">
									<xsl:variable name="nusuario" select="nombre_usuario" />
									<xsl:variable name="npersona" select="nombre_persona" />
									<fila accion="mostrarDetalleCarga('{id_carga}','{id_persona_usuario}','{id_formato}');">
										<valor>
...
<!-- 										</valor> -->
							...
							<formulario id="form_carga" destino="liberacion/15.3.do">
								...
								<variable id="id_formato_entrada" />
							</formulario>
							<xsl:if test="$total != ''">
								<formulario id="form_pag" destino="liberacion/15.2.do">
						...
						<xsl:otherwise>
							<parrafo estilo="resaltado">
								No existen cargas para este proceso
							</parrafo>
					...
					<area_botones>
						<boton estilo="primary volver" destino="liberacion/15.1.do">Volver</boton>
					</area_botones>
