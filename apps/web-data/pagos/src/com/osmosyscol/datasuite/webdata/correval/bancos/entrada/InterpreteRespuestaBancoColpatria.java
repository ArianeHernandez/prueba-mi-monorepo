...
public class InterpreteRespuestaBancoColpatria implements IInterpreteRespuestaBanco {

	private static String nombreBanco = "Colpatria";
	...
	public List<RespuestaBanco> obtenerRespuestasPorBanco(String rutaArchivo, String id_banco) {
		...
		try {
			...
			for (int i=1; i<data.size(); i++) {
				...
				String estado_respuesta = StringUtils.trimToNull(d[10]);

				String num_cuenta = StringUtils.only(d[3], "0123456789");

				String num_doc = Long.parseLong(StringUtils.only(d[4], "0123456789"), 10) + "";

				String valor = StringUtils.only(d[7], "0123456789,").replace(",",".").replace(".00", "");
	...
	public Boolean validarArchivo(String rutaArchivo, String id_banco) {
		try {
			...
			if (StringUtils.trimToEmpty(data.get(0)[10]).equalsIgnoreCase("Estado")) {
				return true;
			}
