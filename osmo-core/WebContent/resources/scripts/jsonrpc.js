...
JSONRpcClient = function JSONRpcClient_ctor(serverURL, user, pass, objectID)
{
    ...
    try {
    	...
    	SimpleLogger.setError("Error en servicio json:" + serverURL, err);
//    	alert("Error inesperado consultando listado de m\u00E9todos JSON.");
    	osm_unblock_window();
    }
}
...
function JSONRpcClient_default_ex_handler(e) {
	SimpleLogger.setError("Error en servicio json", e);
}
...
JSONRpcClient.max_req_active = 20;
...
function createJSONRpcClient(){
	return new JSONRpcClient( CONTEXTPATH + "/JSON-RPC" );
}
