 ...
 * the resulting Java objects to JSON objects in JSON-RPC result format. <p />
 * An instance of the JSONRPCBridge object is automatically placed in the
 * HttpSession object registered under the attribute "JSONRPCBridge" by the
 * JSONRPCServlet. <p /> The bridge is implemented as session specific to
 * improve the security of applications by allowing exporting of object methods
 * only to specific HttpSessions. <p /> To use the bridge to allow calling of
 * Java methods you can easily access the HttpSession specific bridge in JSP
 * using the usebean tag. eg. <p />
 * <code>&lt;jsp:useBean id="JSONRPCBridge" scope="session"
 *   class="com.metaparadigm.jsonrpc.JSONRPCBridge" /&gt;</code> <p /> Then
 * export the object you wish to call methods on. eg. <p />
 * <code>JSONRPCBridge.registerObject("test", testObject);</code> <p /> This
 * will make available all methods of the object as
 * <code>test.&lt;methodnames&gt;</code> to JSON-RPC clients. This method should
 * generally be performed after an authentication check to only export specific
 * objects to clients that are authorised to use them. <p /> There exists a
 * global bridge singleton object that will allow exporting objects to all HTTP
 * clients. This can be used for registering factory classes although care must
 * be taken with authentication as these objects will be accessible to all
 * clients.
 */

public class JSONRPCBridge {
	private final static Logger log = Logger.getLogger(JSONRPCBridge.class.getName());
	...
	static {
		log.info("Creating global bridge");
		globalBridge = new JSONRPCBridge();
		try {

			Configurator.registerServices();
		} catch (Exception e) {
	...
	static {
		registerLocalArgResolver(javax.servlet.http.HttpServletRequest.class, javax.servlet.http.HttpServletRequest.class, requestResolver);
		registerLocalArgResolver(javax.servlet.http.HttpSession.class, javax.servlet.http.HttpServletRequest.class, sessionResolver);
		registerLocalArgResolver(JSONRPCBridge.class, javax.servlet.http.HttpServletRequest.class, bridgeResolver);
	}
	 ...
	 * by default. <p /> The JSONBridge will take a references to these objects
	 * and return an opaque object to the JSON-RPC client. When the opaque
	 * object is passed back through the bridge in subsequent calls, the
	 * original object is substitued in calls to Java methods. This should be
	 * uses for any objects that contain security information or complex types
	 * that are not required in the Javascript client but need to be passed as a
	 * reference in methods of exported objects. <p /> A Reference in JSON
	 ...
	 * Callable references are not explicitly supported by the JSON-RPC. The
	 * implementation in JSON-RPC-Java requires the JSON-RPC client to connect
	 * to a different URL including the returned object_id to call methods on
	 * the reference. The format of the URL is: <p />
	 * <code>/JSON-RPC?object_id=&lt;object_id&gt;</code> <p /> A Callable
	 * Reference in JSON format looks like this: <p />
	 ...
	 * CallableReferences to the JSON-RPC client. <p /> To export instance
	 * methods you need to use registerObject.
	 * 
	 * @param clazz
	...
	private Method resolveMethod(HashMap methodMap, String methodName, JSONArray arguments) {
		...
		for (int i = 0; i < candidate.size(); i++) {
			...
			if (best == null || best.getMatch().mismatch > c.getMatch().mismatch)
				best = c;
		}
		if (best != null) {
	...
	@SuppressWarnings("unchecked")
	protected JSONRPCResult call(Object context[], int object_id, String classDotMethod, JSONArray arguments) {
		...
		if (object_id == 0) {
			...
			if (classDotMethod.equals("system.listMethods")) {
				...
				if (globalBridge != this) { // sesion
					...
					allStaticMethods(m2);
					allInstanceMethods(m2);
					
					HttpSession httpSession = ((HttpServletRequest) context[0]).getSession();
					
					for (Object methodSession : m2) {
						String urn = methodSession.toString().split("[.]")[0];
						String methodNameStr = methodSession.toString().split("[.]")[1];
						
						String classNameStr = null;
						Class classs = (Class) classMap.get(urn);
						
						if(classs != null){
							classNameStr = classs.getCanonicalName();
							
						}else{
							Object object = objectMap.get(urn);
							if(object!=null){
								classNameStr = ((ObjectInstance)object).classData().clazz.getCanonicalName();
							}
						}
						...
						if(classNameStr!=null){
							...
							if(JSONCallBackAutorizacion.AUTORIZAR_JSON){
								addMethod = AutenticacionServicio.getInstance().autorizarUrl( (Session) httpSession.getAttribute("cocoon-session")  , classNameStr + "." +  methodNameStr);
							}else{
								addMethod = true;
							}
		...
		try {
			...
			SerializerState state = new SerializerState();
			result = new JSONRPCResult(JSONRPCResult.CODE_SUCCESS, ser.marshall(state, o));
		} catch (UnmarshallException e) {
			result = new JSONRPCResult(JSONRPCResult.CODE_ERR_UNMARSHALL, e.getMessage());
		} catch (MarshallException e) {
			result = new JSONRPCResult(JSONRPCResult.CODE_ERR_MARSHALL, e.getMessage());
		} catch (Throwable e) {
