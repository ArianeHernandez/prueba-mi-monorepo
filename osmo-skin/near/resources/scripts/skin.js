...
var SKIN_TIEMPO_INACTIVIDAD_MAX = 60 * 720; // 12 horas
		...
		osm_timer(function() {
			...
			if (tt > 80) {
				if (!b_mensaje) {
					...
					if (ts == 300) {
						s_alert_warn("Cierre de sesi\u00f3n por inactividad",
							"La sesi\u00f3n del M\u00f3dulo de Insolvencia se cerrar\u00e1 en los pr\u00f3ximos cinco minutos, aseg\u00farese de haber finalizado las operaciones.",
							cambiarIconoDefault);
						
						if (notificarInactividad) {
...
function skin_validar_tipo(id_obj, tipo, obligatorio) {
	...
	if (tipo == "DECIMALSIGNO" && !osm_validarDecimalConSigno(valor)) {
		return false;
	}
