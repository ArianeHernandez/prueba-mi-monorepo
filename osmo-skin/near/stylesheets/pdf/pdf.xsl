...
<xsl:stylesheet version="1.0"
	...
	<xsl:decimal-format name="formato-moneda-col" decimal-separator="," grouping-separator="." />
			...
			<fo:table space-before="5mm" space-after="5mm" border-width="0.2pt"  border-style="solid" border-left-color="#FFFFF8" border-right-color="#FFFFF8" border-top-color="#FFFFF8">
				...
				<fo:table-body>
	                <fo:table-row>
		              ...
		              <fo:table-cell border-width="0.2pt" border-color="#000000" border-style="solid" border-left-color="#FFFFFF" border-right-color="#FFFFFF" border-top-color="#FFFFFF">
		              	<fo:block font-size="12pt" font-weight="bold" text-align="start"  font-family="times" padding-top="30pt" padding-buttom="1pt">
							<xsl:value-of select="titulo"/>
						</fo:block>    
		              </fo:table-cell>
	...
	<xsl:template match="titulo">
		<fo:block font-size="16pt" padding-top="20pt" padding-buttom="5pt" text-align="left" font-family="arial" font-color="#094EB6">
			<xsl:value-of select="."/>
		</fo:block>
	</xsl:template>

	<xsl:template match="subtitulo">
		<fo:block font-size="12pt" font-weight="bold" color="#00467d" padding-top="5pt" padding-buttom="5pt" text-align="start" font-family="verdana">
			<xsl:value-of select="."/>
		</fo:block>
	</xsl:template>
	
	<xsl:template match="subtitulo2">
		<fo:block font-size="14pt" font-weight="bold" color="#00467d" padding-top="5pt" padding-buttom="5pt" text-align="start" font-family="times">
			<xsl:value-of select="."/>
		</fo:block>
	</xsl:template>
	
	<xsl:template match="moneda">
		<fo:block font-size="12pt" padding-buttom="5pt" text-align="start" font-family="times">
			<xsl:value-of select= 'format-number(number(.), "$###.###,00","formato-moneda-col")' />
		</fo:block>
	</xsl:template>
	...
	<xsl:template match="tabla_solicitud">
		...
		<fo:table space-before="5mm" border-width="0.2pt"  border-style="solid" border-color="#9B9B9B"  width="100%"> 
			...
			<!-- columnas titulo -->
			<xsl:for-each select="encabezado/titulo">
				<xsl:variable name="posColumna"  select="position()"></xsl:variable>
				<xsl:if test="$posColumna mod 2 = 0">
	                 	...
	                 	<xsl:for-each select="valor">
	                      ...
	                      <xsl:if test="$pos mod 2 != 0">
	                      	<xsl:attribute name="font-weight">bold</xsl:attribute>
	                           ...
	                           <fo:block font-size="12pt" margin="5pt" padding-top="5pt" padding-buttom="5pt" font-family="times" text-align="left">
	                           		<xsl:apply-templates/>
	                           </fo:block>
	                      </fo:table-cell>
	                    </xsl:for-each>
