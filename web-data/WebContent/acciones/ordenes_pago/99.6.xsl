...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="OSM-ACCION">
		<pagina titulo="Consultar Órdenes de Pago Automáticas">
			...
			<javascript>ordenes_pago/99.6.js</javascript>
			...
			<principal>
				...
				<contenido>
					...
					<xsl:for-each select="//obtenerEstados/Listado/Estado">
						<variable id="estado_{estado}" valor="{nombre_interno}" />
					</xsl:for-each>
					<variable id="estado_" valor="--" />
					...
					<bloque estilo="col panel panel-default">
						...
						<div class="panel-body">
							<div class="panel-col col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2 form-horizontal">
								...
								<registro>
									<item>Estado del Lote</item>
									<valor>
										<cajatextoselector id="estado_carga">
											<opcion valor="" texto="-- Seleccione --"/>
											<xsl:for-each select="//obtenerEstados/Listado/Estado">
												<xsl:sort select="nombre_interno" />
												<opcion valor="{estado}" texto="{nombre_interno}" />
											</xsl:for-each>
										</cajatextoselector>
									</valor>
								</registro>
						...
						<div class="panel-footer">
							<boton estilo="default btn-sm" accion="limpiarFiltros();"><i class="fa fa-eraser" aria-hidden="true"/>&#160;Limpiar</boton>
							<boton estilo="primary btn-sm" accion="PAGINA_ACTUAL = 1; contarArchivos();"><i class="fa fa-search" aria-hidden="true"/>&#160;Buscar</boton>
						</div>
					</bloque>
					
					<variable id="totalArchivos" valor="{totalArchivos}" />
					<variable id="id_usuario" valor="" />
						...
						<div class="tabla_borde table-responsive">
								...
								<tbody id="archivos_obtenidos">
									...
									<tr>
										...
										<td>
											<xsl:variable name="estado_carga" select="estado_carga"/>
											<xsl:for-each select="//obtenerEstados/Listado/Estado[estado=$estado_carga]">
												<xsl:value-of select="nombre_interno" />
											</xsl:for-each>
											<xsl:if test="count(estado_carga)=0">--</xsl:if>
										</td>
										<td>
					...
					<div class="alert alert-info" id="div_no_resultados"><i class="fa fa-info-circle" aria-hidden="true"></i>
...
<!-- 					<area_botones> -->
<!-- 						<boton estilo="default volver" destino="inicio/0.do">Ir al Inicio</boton> -->
<!-- 					</area_botones> -->
					</div>
				</contenido>
			</principal>
