package com.osmosyscol.datasuite.webdata.correval.siforion.salida.servlet;
...
public class DescargaArchivoSIFORIONServlet extends HttpServlet {
	...
	private void doDownload(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String ruta_archivo = session.getAttribute("archivo_sif_orion").toString();
			String tipo_archivo = session.getAttribute("tipo_archivo_sif_orion").toString();
...
//			
			...
			if(file!=null && file.exists()){
				...
				response.setHeader("Content-Disposition", "attachment; filename=\"" + tipo_archivo +".xls"+ "\"");
