...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="OSM-ACCION">
		...
		<pagina titulo="Cargue HTS">

			<principal>
				<titulo>Cargue HTS</titulo>
				<contenido>
					
					<javascript>htsa/htsa_1.js</javascript>
					...
					<xsl:choose>
						<xsl:when test="//cargarRetirosInfo/registro/num_retiros > 0">
							...
							<form id="form_enviarcarga" action="HTSA_2.do">
							...
							<xsl:when test="//cargarRetirosInfo/registro/exitoso = 'true'">
									<xsl:choose>
										<xsl:when test="(//OSM-INIT-SESSION/Info/Usuario/uso_firma='S' and //esclientenatural='true') or (//OSM-INIT-SESSION/Info/Usuario/uso_firma_preparador='S')">
											...
											<xsl:with-param name="id_componente_firma">firmaEnviarHTSd1</xsl:with-param>
											...
											<xsl:with-param name="nombre_boton">Firmar y Enviar HTS</xsl:with-param>
											<xsl:with-param name="rutaBotonVolver">carga_informacion/bancos/HTSA.do</xsl:with-param>
											</xsl:call-template>
								...
								<xsl:if test="not(//OSM-INIT-SESSION/Info/Usuario/uso_firma='S' and //esclientenatural='true') and not(//OSM-INIT-SESSION/Info/Usuario/uso_firma_preparador='S')">
									<boton estilo="danger" destino="carga_informacion/bancos/HTSA.do">Cancelar</boton>
								</xsl:if>
							</xsl:when>
							<xsl:otherwise>
								<boton estilo="primary volver" destino="carga_informacion/bancos/HTSA.do">Volver</boton>
							</xsl:otherwise>
