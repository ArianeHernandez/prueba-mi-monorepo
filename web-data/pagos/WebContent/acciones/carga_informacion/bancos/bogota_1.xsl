...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="OSM-ACCION">
		...
		<pagina titulo="Cargue Bogota">

			<principal>
				<titulo>Cargue Bogota</titulo>
				<contenido>
					
					<javascript>bogota/bogota_1.js</javascript>
					...
					<xsl:choose>
						<xsl:when test="//cargarRetirosInfo/registro/num_retiros > 0">
							...
							<form id="form_enviarcarga" action="bogota_2.do">
									<!-- INICIO INFORMACION DE LA TRANSACCION -->
									<input type="hidden" name="transaccion_occired" value="{ID_CARGA}"/>
									...
									<input type="hidden" name="enviarOccired" value="S"/>
							...
							<xsl:when test="//cargarRetirosInfo/registro/exitoso = 'true'">
									<xsl:choose>
										<xsl:when test="(//OSM-INIT-SESSION/Info/Usuario/uso_firma='S' and //esclientenatural='true') or (//OSM-INIT-SESSION/Info/Usuario/uso_firma_preparador='S')">
											...
											<xsl:with-param name="id_componente_firma">firmaEnviarBogotad1</xsl:with-param>
											...
											<xsl:with-param name="nombre_boton">Firmar y Enviar Bogota</xsl:with-param>
											<xsl:with-param name="rutaBotonVolver">carga_informacion/bancos/bogota.do</xsl:with-param>
											</xsl:call-template>
										
										</xsl:when>
										
										<xsl:otherwise>
												<boton estilo="enviar" formulario="form_enviarcarga" validacion="validacion_carga_archivo()">Enviar</boton>
										</xsl:otherwise>
								...
								<xsl:if test="not(//OSM-INIT-SESSION/Info/Usuario/uso_firma='S' and //esclientenatural='true') and not(//OSM-INIT-SESSION/Info/Usuario/uso_firma_preparador='S')">
									<boton estilo="danger" destino="carga_informacion/bancos/bogota.do">Cancelar</boton>
								</xsl:if>
							</xsl:when>
							<xsl:otherwise>
								<boton estilo="primary volver" destino="carga_informacion/bancos/bogota.do">Volver</boton>
							</xsl:otherwise>
