		...
		<pagina titulo="Cargue Occidente">
			...
			<principal>
				...
				<contenido>
					...
					<javascript>occired/occired_1.js</javascript>
					...
					<xsl:choose>
						<xsl:when test="//cargarRetirosInfo/registro/num_retiros > 0">
							...
							<bloque-pestanas>
								<pestana titulo="Cargue Occidente">
							...
							<form id="form_enviarcarga" action="occired_2.do">
							...
							<xsl:when test="//cargarRetirosInfo/registro/exitoso = 'true'">
									<xsl:choose>
										<xsl:when test="(//OSM-INIT-SESSION/Info/Usuario/uso_firma='S' and //esclientenatural='true') or (//OSM-INIT-SESSION/Info/Usuario/uso_firma_preparador='S')">
											...
											<xsl:with-param name="nombre_boton">Firmar y Enviar Occired</xsl:with-param>
											<xsl:with-param name="rutaBotonVolver">carga_informacion/bancos/occired.do</xsl:with-param>
											</xsl:call-template>
								...
								<xsl:if test="not(//OSM-INIT-SESSION/Info/Usuario/uso_firma='S' and //esclientenatural='true') and not(//OSM-INIT-SESSION/Info/Usuario/uso_firma_preparador='S')">
									<boton estilo="danger" destino="carga_informacion/bancos/occired.do">Cancelar</boton>
								</xsl:if>
							</xsl:when>
							<xsl:otherwise>
								<boton estilo="primary volver" destino="carga_informacion/bancos/occired.do">Volver</boton>
							</xsl:otherwise>
