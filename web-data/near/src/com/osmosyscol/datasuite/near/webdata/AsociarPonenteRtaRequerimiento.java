...
public class AsociarPonenteRtaRequerimiento implements AccionCarga {
		...
		try {
			...
			if (solicitud == null) {
				...
				if (carga_instancias != null && carga_instancias.size() > 0) {
					...
					List<Integer> personas = AccionResponsableServicio.getInstance().obtenerResponsablesPorCarga(solicitud.getNumero_solicitud());
					...
					if (acciones != null && acciones.size() > 0 && personas != null && personas.size() > 0) {
						...
						SimpleLogger.setError("No se encontraron acciones o persona responsable asociada "
								+ "a la solicitud padre " + solicitud.getNumero_solicitud() + " de la respuesta con id " + id_carga);
