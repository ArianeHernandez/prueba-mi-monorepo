...
public class NotificarNumeroRadicadoRegimen implements AccionCarga{
	
	@Override
	public SMessage ejecutar(Integer id_carga) {
		RegimenInsolvencia solicitud = null;
		try {
			
			solicitud = RegimenInsolvenciaFetchBuilder.newInstance().obtenerInfoRegimenInsolvenciaPorIdCarga(id_carga);
			
			if (solicitud == null) {
				SimpleLogger.setError("No se encuentra una solicitud de Otros Procesos para la carga " + id_carga);

				return new SMessage(false, "");
			}
			//enviar notificacion
			Deudor deudor = solicitud.getDeudor();
			...
			if (solicitud.getIntendencia_regional_id() != null) {
				nombre_archivo = ContenidoServicio.getInstance().obtenerContenido("General", "ARCHIVO_NOTIFICACION_RADICADO_" + solicitud.getIntendencia_regional_id());				
			} else {
			...
			mensaje = mensaje.replace("#numero_radicado#", solicitud.getNumero_radicado());
			mensaje = mensaje.replace("#nombre_proceso#", solicitud.getTipo_solicitud_obj().getNombre());
			...
			TokenMapping[] tokens = new TokenMapping[]{
				...
				new TokenMapping("@{intendencia}", solicitud.getIntendencia_regional().getNombre()),
				new TokenMapping("@{nombre_proceso}", solicitud.getTipo_solicitud_obj().getNombre()),
				new TokenMapping("@{numero_radicado}", solicitud.getNumero_radicado())
			};
			...
			if (solicitud.getIntendencia_regional_id() != null) {
				nombre_archivo_salida = ContenidoServicio.getInstance().obtenerContenido("General", "ARCHIVO_NOTIFICACION_RADICADO_SALIDA_" + solicitud.getIntendencia_regional_id());				
			} else {
			...
			EnviaMails.enviar(deudor.getCorreo_electronico(), deudor.getRazon_social(), "Radicación " + solicitud.getTipo_solicitud_obj().getNombre(),
